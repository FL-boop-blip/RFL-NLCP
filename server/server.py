...
class Server(object):
    def __init__(self, device, model_func, init_model, init_par_list, datasets, method, args):
        ...
        self.Client = None
        root = '{:s}/{:s}/T={:d}'.format(self.args.out_file, self.args.method,self.args.comm_rounds)
        if not os.path.exists(root):
            os.makedirs(root)
        if not self.args.non_iid:
            root += '/{:s}-{:s}{:s}-{:d}'.format(self.args.dataset, 'IID', 
                                                 '', self.args.total_client)
        elif self.args.method == 'Ada_FedPD' or self.args.method == 'Ada_FedPD_SAM':
            root += '/{:s}-{:s}{:s}-{:d}-{:s}-{:s}'.format(self.args.dataset, self.args.split_rule, 
                                                 str(self.args.split_coef), self.args.total_client, str(self.args.alpha1), str(self.args.rho))
        else:
    ...
    def _test_(self, t, selected_clients):
        ...
        if self.args.mode=="dg":
            ...
            if acc>self.best:
                self.best=acc
                #torch
                save_file = os.path.join(self.root, 'Model', self.args.method)
                if not os.path.exists(save_file):
                    os.makedirs(save_file)
                torch.save(self.server_model.state_dict(), os.path.join(save_file,'best.pth'))
    ...
    def _summary_(self):
        ...
        "##                   Summary                   ##",
        "##=============================================##",
        "     Communication round   --->   T = {:d}       ".format(self.args.comm_rounds),
        "    Average Time / round   --->   {:.2f}s        ".format(np.mean(self.time)),
        "     Top-1 Test Acc (T)    --->   {:.2f}% ({:d}) ".format(np.max(self.test_perf[:,1])*100., np.argmax(self.test_perf[:,1]))
        ]

        for line in summary_lines:
            print(line)
        os.makedirs(self.root, exist_ok=True)
        output_file = os.path.join(self.root, 'summary.txt')
    ...
    def _save_results_(self):
        ...
        time_file = time_dir + '/time-{:s}{:s}.npy'.format(self.args.method, options)
