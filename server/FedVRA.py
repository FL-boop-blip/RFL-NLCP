...
class FedVRA(Server):
    def __init__(self, device, model_func, init_model, init_par_list, datasets, method, args):   
        super(FedVRA, self).__init__(device, model_func, init_model, init_par_list, datasets, method, args)
    ...
    def global_update(self, selected_clients, Averaged_update, Averaged_model):
        ...
        return self.args.active_ratio * self.args.d * Averaged_model + torch.mean(self.h_params_list, dim=0)
    
    
    def postprocess(self, client,received_vecs):
        self.h_params_list[client] += self.args.a * self.clients_updated_params_list[client]