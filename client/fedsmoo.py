...
class fedsmoo(Client):
    def __init__(self, device, model_func, received_vecs, dataset, lr, args):   
        ...
        self.optimizer = DRegSAM(self.model.parameters(), self.base_optimizer, rho=self.args.rho,dataset=self.args.dataset)
        ...
        if self.args.dataset == 'AG_News':
            for k in range(self.args.local_epochs):
                for i, (inputs, labels,lengths) in enumerate(self.dataset):
                    ...
                    self.optimizer.paras = [inputs, labels, lengths, self.loss, self.model, self.received_vecs['Dynamic_dual_correction']]
